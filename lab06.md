...
CustomerChargesDetail.csv on your workstation. We will now upload this
...
Click the “+ New folder” button and create a new folder called
**ChargesDetail**.
...
Double click the new ChargesDetail folder. Then click the Upload button:

<img src="images/lab06/media/image7.png" style="width:4.35477in;height:2.09404in" alt="Graphical user interface, text, application, email Description automatically generated" />

Choose the CustomerChargesDetail.csv file you previously downloaded and
click Upload.
...
Go to the Integrate tab (the pipeline icon) in Synapse Studio:
...
Click the + sign and choose Pipeline to create a new Synapse pipeline.
...
In the Properties pane on the right, rename the pipeline and provide a
description:
...
With the Data flow module selected, view the General tab at the bottom
...
On the Settings tab, click the +New button to create a new Mapping data
flow:
...
In the properties pane rename to “dfRefreshCustomerCharges” (“df” stands
for data flow):
...
In the “Source settings” tab at the bottom name it
...
On the “Source options” tab click the Browse button on the File path:
...
Browse to staging/ChargesDetail and select CustomerChargesDetail.csv and
click OK:
...
Check the “first row as header” checkbox:
...
On the Projection tab, notice the “Import schema” button is grayed out.
...
Accept the defaults and click OK. The debug time to live of 1 hour means
...
Wait about 2-5 minutes until the data flow debug compute spins up. When
...
Click the Import schema button now. Accepting the default formats is
...
Now the schema for this CustomerChargesDetail source is populated.
...
Go to the **Data preview** tab and click the **Refresh** button to see a
...
This file has one row per service per customer per month. We want to
...
Describe what you are doing in the Output stream name, set the column
...
Click the + sign next to the SumChargesAndTax transform and add
Aggregate:
...
Name the output stream AggregateByCustomerMonth (there is only one
...
Then click the “Aggregates” selector and add a TotalCharges column with
...
Click the + sign next to the AggregateByCustomerMonth and choose
**Select**:<img src="images/lab06/media/image33.png" style="width:5.19864in;height:4.6569in" alt="Graphical user interface, application Description automatically generated" />

The column order looks like this to begin with:
...
Rename the TotalCharges column to just Charge and mouse over that row
...
Go to the Data preview tab on the Select1 module and click Refresh.
...
Now let’s write out the results. Click the + icon next to the Select1
...
Name the output stream CustomerCharges, choose an Inline sink type,
...
Choose the staging container and the Charges folder. Check the “first
...
On the Optimize tab of the CustomerCharges sink you can keep the current
...
filename. If your scenario requires a single filename then “Single
partition”
...
Flip back to the RefreshCustomerCharges pipeline tab, click the Debug
...
Go to the Data left nav, the Linked tab, choose the primary storage
...
Click the Publish all button to save the pipeline and mapping data flow
...
Confirm the list of objects to publish and click the Publish button at
the bottom:
...
On the Integrate left nav, open the RefreshCustomerChargesAndChurnML
...
Drag the green square from the Refresh Customer Charges data flow module
...
Click on the ML module and rename it Execute Churn ML Pipeline:
...
On the Settings tab for the ML module, click the new button to create a
...
Name the linked service “lsAzureML” (the “ls” prefix stands for Linked
...
Click “Test connection” and you should see a “Connection successful”
...
Change the Pipeline ID type to “Pipeline endpoint ID”, choose the
...
Click the Debug button at the top of the
...
Confirm the two objects which are changed:
...
Click the Add trigger button at the top of the
...
On the Add triggers pane, choose +New from the dropdown:
...
Name the trigger Daily3am. Set the Start date to 3:00AM tomorrow. Choose
...
Click Publish all and confirm the trigger to be published. Click
Publish.
...
If you ever want to adjust the trigger, go to the Manage left nav,
...
At <https://home.ci.ai.dynamics.com/> on the Data… Data sources left
...
Choose “Refresh automatically” and set it to refresh on specific days
