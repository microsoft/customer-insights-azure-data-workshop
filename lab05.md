...
Browse to <https://portal.azure.com> and sign in with your
...
On the Security + networking… Access keys blade, click the “Show keys”
button:
...
Choose Import data and give it the name “AzureMlResults” and click Next.
...
Click the Azure tab in the Power Query Online window and click the Azure
Blobs option:

<img src="images/lab05/media/image8.png" style="width:7.5in;height:3.08681in" />

In the “Account name or URL” type in the name of your storage account
...
your asastore\<suffix> storage account.
...
Click the checkbox on the container called “azureml-blobstore-\<guid>”
...
Paste “azureml/Decision_Tree_Results/dtresults/data.csv” into the Filter
...
Power Query Online should now show one row with the CSV file which is
...
The columns in your ML results file should show on the screen. On the
...
The first row has now been promoted to column headers. Now click the ABC
...
Name the new column ChurnRisk and setup the conditions as follows. If
...
Click the radio button to the left of the AzureMlResults data source and
...
Click the checkmark next to the PredictedCustomerChurn entity and click
Apply:
...
Click the PredictedCustomerChurn entity and set the primary key to
...
Go to the Match tab and click the “+Add entity” button and choose the
...
Click the “+Add rule” button under the AzureMlResults :
PredictedCustomerChurn entity:
...
Choose the CustomerDemographics entity and the customerID fields on both
...
Click the arrow next to Scored Labels, Scored Probabilities, and
...
Scroll down to the bottom of the field list, mouse over the Scored
...
Once the run is successful, go to the Data… Entities left nav, click on
...
Go to the Customers left nav and click “Search & filter index”. Let’s
...
Check the Churn, PredictedChurn and ChurnRisk fields (decimal fields are
...
Mouse over the **ChurnRisk** field and click the **funnel icon** to
...
When prompted after clicking the funnel icon, accept the default options
and click **OK**.
...
Then click the **Save** and **Run** buttons. When refreshing completes,
...
Click the **Save filters as segment** link and enter a name for this new
...
Go to the Segments left nav, select the Customers at High Risk of Churn
