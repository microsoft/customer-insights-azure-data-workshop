...
Choose Azure Synapse Analytics from the dropdown. Note that the Azure
...
On the “Set up Azure Synapse Analytics export” pane, choose the
...
Ensure the “Connect to” dropdown has “Built-in” (meaning the Synapse
...
Highlight the current query and press the backspace key to delete the
...
link and choose “Save link as…” and name the file CustomerCharges.csv on
...
Choose the CustomerCharges.csv file you previously downloaded and click
Upload.
...
Right click on the CustomerCharges.csv file that has been uploaded to
...
To fix the header row, paste in the following query and fix \<suffix>
then run the query: 
